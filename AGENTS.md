...
- **`appwrap dev <ios|android> [--url <url>]`** — live-dev. **android** → `ns run` livesync (true on-device HMR) + a watcher that rebuilds web & re-stages www on save (pre-installs via deploy `--user 0` so ns's install lands as an update). **iOS** → deploy + attach + rebuild/reinstall on save (not `ns run`: device signing path pending verify). `--url` points the shell at a live dev server (loader `server`, web HMR inside the WebView; reverts on the next `sync`). Dev server must bind `0.0.0.0`. `--sim` = ns run/HMR on emulator; `--detached` = deploy + exit; `--debug` = android `ns debug` / iOS open the inspector. **Wireless (android):** `--wifi` flips a USB device to wireless adb once; thereafter it's auto-discovered (remembered-reconnect + mDNS), so plain `dev android` runs cordless with no flag — parity with how `devicectl` lists a network iPhone. `--device <ip[:port]>` targets a specific one.
...
Always-on core (haptics, storage, toast, share, ui, device, clipboard, network, lifecycle) + opt-in modules: `notifications`, `biometrics`, `geo`, `photos`, `camera`, `media` (mic/camera/speaker), `motion`, `contacts`, `calendar`, `reviews`, `billing`, `health`, `oauth`, `widget` (home-screen WidgetKit / AppWidget), `shareTarget` (INBOUND OS share sheet — Android ACTION_SEND intent-filter + iOS generated share-extension target that persists into an App-Group mailbox drained on launch/resume (iOS forbids extensions launching the host app), delivered as a `<urlScheme>://share?…` deep link → `kit.shareTarget.onReceive`; requires `urlScheme`), plus remote `push`. Also `kit.app.pinShortcut` (Android standalone home-screen icon). Always branch on `kit.<domain>.capability` and handle the `'web'`/`'none'` fallbacks.
