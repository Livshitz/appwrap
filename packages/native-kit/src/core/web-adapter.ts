      ...
      case 'motion.start': {
        ...
        if (!this.motionHandler) {
          ...
          this.motionHandler = (e) => {
            ...
            // Map DOM → kit contract. Two corrections vs the raw event, BOTH load-bearing:
            // 1. UNITS: the DOM reports rotationRate in deg/s; the kit contract (and both native
            //    handlers — CoreMotion + Android TYPE_GYROSCOPE) is rad/s. Convert, or web is 57× off.
            // 2. AXES: DOM alpha/beta/gamma are the rates about Z/X/Y respectively — NOT x/y/z in
            //    order. So rx=beta, ry=gamma, rz=alpha; a naive alpha→rx puts the Z-rate on X.
            const toRad = (d?: number | null) => (d == null ? undefined : (d * Math.PI) / 180);
            this.emit('motion.data', {
              ax: a?.x ?? 0, ay: a?.y ?? 0, az: a?.z ?? 0,
              rx: toRad(r?.beta), ry: toRad(r?.gamma), rz: toRad(r?.alpha),
            });
          };
