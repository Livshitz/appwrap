...
describe('stampPrivacyTracking — ATT declarations into the real store-readiness manifest', () => {
  ...
  test('module INACTIVE: NSPrivacyTracking stays false, domains empty (template default preserved)', () => {
    ...
    // Default template collects NOTHING (matches a "Data Not Collected" store label); the
    // tracking stamper does not add collected-data types (apps that collect declare via overrides).
    expect(out).toContain('<key>NSPrivacyCollectedDataTypes</key>\n\t<array/>');
    expect(out).not.toContain('NSPrivacyCollectedDataTypeDeviceID');
  });
