...
export function resolveKeyboardHeight(input: KeyboardFrameInput): KeyboardFrameDecision {
  ...
  // Overlap with the container, NOT end.height: iOS delivers keyboard-sized but off-screen end
  // frames during dismissal — end.height would shrink a keyboard-less screen.
...
export function createKeyboardShowCycle() {
  ...
  return {
    resolve(input: Omit<KeyboardFrameInput, 'inShowCycle'>): KeyboardFrameDecision {
      if (input.tag !== 'willChangeFrame') open = true;
      return resolveKeyboardHeight({ ...input, inShowCycle: open });
    },
    close(): void {
      open = false;
    },
