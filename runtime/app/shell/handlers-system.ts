import { Application, ApplicationSettings, Color, ImageSource, Utils, isAndroid, isIOS } from '@nativescript/core';
...
export function registerSystemHandlers(): void {
  ...
  // Pin a STANDALONE launcher icon to the home screen (distinct from setShortcuts' long-press menu).
  // Android 8+ (API 26) ShortcutManager.requestPinShortcut — the system shows the add-to-home-screen
  // prompt. iOS has NO equivalent API (a pinned mini-app icon there is served by the widget instead),
  // so the capability map gates this to android-only and we return {pinned:false} defensively.
  // `url` is the full deep link (e.g. blank://a/<id>) supplied by the caller — keeps this app-agnostic;
  // the tap flows through the app's existing deep-link routing. `iconUrl` is fetched to a bitmap tile.
  bridge.register('app.pinShortcut', async ({ id, title, url, iconUrl, subtitle }: { id: string; title: string; url: string; iconUrl?: string; subtitle?: string }) => {
    if (!id || !title || !url) throw err('NATIVE_ERROR', 'app.pinShortcut: id, title, url required');
    if (!isAndroid) return { pinned: false, reason: 'unsupported' as const };
    if (android.os.Build.VERSION.SDK_INT < 26) return { pinned: false, reason: 'unsupported' as const };
    const ctx = Utils.android.getApplicationContext();
    const sm = ctx.getSystemService(android.content.Context.SHORTCUT_SERVICE);
    if (!sm || !sm.isRequestPinShortcutSupported()) return { pinned: false, reason: 'unsupported' as const };

    const intent = new android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse(String(url)));
    const launchClass = (Application.android.foregroundActivity ?? Application.android.startActivity)?.getClass();
    if (launchClass) intent.setClassName(ctx, launchClass.getName());
    intent.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK | android.content.Intent.FLAG_ACTIVITY_CLEAR_TOP);

    const b = new android.content.pm.ShortcutInfo.Builder(ctx, 'pin_' + String(id))
      .setShortLabel(String(title))
      .setLongLabel(String(subtitle ?? title))
      .setIntent(intent);

    // Tile icon: fetch iconUrl → bitmap. On any failure fall back to the app launcher icon (never
    // block the pin on a network hiccup). ImageSource.fromUrl resolves off the main thread.
    let iconSet = false;
    if (iconUrl) {
      try {
        const img = await ImageSource.fromUrl(String(iconUrl));
        if (img?.android) {
          b.setIcon(android.graphics.drawable.Icon.createWithBitmap(img.android));
          iconSet = true;
        }
      } catch (e) {
        console.log('app.pinShortcut: icon fetch failed, using launcher icon: ' + e);
      }
    }
    if (!iconSet) {
      const resId = ctx.getApplicationInfo().icon;
      if (resId) b.setIcon(android.graphics.drawable.Icon.createWithResource(ctx, resId));
    }
