 ...
 *  • scheme      — the app's own custom URL scheme (from `SHELL_CONFIG.urlScheme`), so a webapp inside the
 *                  shell can build its own deep-link URLs. OMITTED when no urlScheme is configured.
 */
...
export function detectEnv(): AppwrapEnv {
  const base: AppwrapEnv = isAndroid
  ...
  // Only surface `scheme` when configured — contract requires the key ABSENT for an empty urlScheme.
  if (SHELL_CONFIG.urlScheme) base.scheme = SHELL_CONFIG.urlScheme;
  return base;
}
