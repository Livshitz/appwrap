 ...
 *
 * Built lazily INSIDE createUiDelegate (mirrors banner.ts): NSObject/WK* are iOS globals, and although
 * this module is reached only from the iOS WebView path today, defining the class at top level would
 * still evaluate those globals at ES-module instantiation — keep it inside the factory to stay safe.
 */
// any: runtime-built ObjC subclass holder; the class BODY stays fully typed.
let AppwrapUiDelegate: any;
function appwrapUiDelegateClass(): any {
  if (AppwrapUiDelegate) return AppwrapUiDelegate;
  @NativeClass()
  class AppwrapUiDelegateImpl extends NSObject implements WKUIDelegate {
    static ObjCProtocols = [WKUIDelegate];
    static new(): AppwrapUiDelegateImpl {
      return <AppwrapUiDelegateImpl>super.new();
    }
  ...
  AppwrapUiDelegate = AppwrapUiDelegateImpl;
  return AppwrapUiDelegate;
}

export function createUiDelegate(): WKUIDelegate {
  return appwrapUiDelegateClass().new();
}
