...
kit.shareTarget.onReceive(({ text, title, files }) => …); // INBOUND share sheet (opt-in `shareTarget` module, requires `urlScheme`): the app appears as an OS share target; Android via an ACTION_SEND intent-filter, iOS via a generated `AppwrapShare` share-extension target that persists the payload into an App-Group mailbox drained by the app on launch/foreground (iOS forbids a share extension from launching its host, so the app receives the share the next time it opens). Delivery rides the deep-link path as `<urlScheme>://share?text=…&title=…&file=…` (cold + warm); `files` are app-cache-relative paths (`kit.fs.read(p, { dir: 'cache', encoding: 'base64' })`)
...
**Hybrid fallback inside a shell.** A native shell doesn't have to implement everything:
capability keys the shell *omits* from its handshake are backed by the kit's web adapter and
surface as `'web'` (and a shell handler that rejects `UNSUPPORTED` is retried against the web
APIs). An **explicit `'none'` from the shell is a veto** — it means the platform genuinely
can't (e.g. `motion` on desktop, where the event API exists but no accelerometer ever fires),
so the kit won't dishonestly upgrade it.
...
- [x] CI/CD templates — GH Actions (PR build + tag→TestFlight; opt-in via `appwrap init --ci`) + fastlane (match, ASC API key)
